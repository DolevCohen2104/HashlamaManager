...
import { getWhatsAppLink, isMammashRole } from '../utils';
...
export default function Dashboard({ profile }: Props) {
    ...
    try {
      ...
      if (normalizedRole !== 'צוער' && evts.length > 0 && !selectedEventId) {
        ...
        if (currentEvent) {
          ...
          if (isMammashRole(normalizedRole)) {
  ...
  const isMammash = isMammashRole(normalizedRole);
  const isStaff = ['מפק"צ', 'סמק"ס', 'מק"ס', 'מה"מ'].includes(normalizedRole) || isMammashRole(normalizedRole);
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in">
      ...
      {!isMammashRole(normalizedRole) && normalizedRole !== 'מה"מ' && hasActiveRollCalls && (
      ...
      {(() => {
        const hasTasksTab = normalizedRole === 'מה"מ' || (isMammashRole(normalizedRole) && tasksEnabled);
        const hasMifkadTab = normalizedRole === 'מה"מ' || (isMammashRole(normalizedRole) && hasActiveRollCalls);
        ...
        return (
          ...
          {(normalizedRole === 'מה"מ' || (isMammashRole(normalizedRole) && tasksEnabled)) && (
            <button 
          ...
          {(normalizedRole === 'מה"מ' || (isMammashRole(normalizedRole) && hasActiveRollCalls)) && (
            <button 
