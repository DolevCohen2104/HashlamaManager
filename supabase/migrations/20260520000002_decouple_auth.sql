-- 1. Create a helper function to extract personal_id from the authenticated user's email
CREATE OR REPLACE FUNCTION public.auth_personal_id() RETURNS text AS $$
  SELECT split_part(auth.jwt()->>'email', '@', 1);
$$ LANGUAGE sql STABLE;

-- 2. Drop existing policies that depend on public.users.id
...
-- 3. Modify public.users table to remove `id` and make `personal_id` the PK
...
-- 4. Recreate Policies using the new auth_personal_id() function
